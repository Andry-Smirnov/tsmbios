...
begin
  ...
  case GetTypeData(Info)^.OrdType of
    otSByte,
    otUByte: Result := Byte(SetParam);
    otSWord,
    otUWord: Result := Word(SetParam);
    otSLong,
    otULong: Result := Integer(SetParam);
  end;
end;
    ...
    if SMBios.HasProcessorInfo then
      for LProcessorInfo in SMBios.ProcessorInfo do
        begin
          ...
          if (LProcessorInfo.RAWProcessorInformation^.L1CacheHandle > 0)
            and (LProcessorInfo.L2Chache <> nil)  then
            begin
          ...
          if (LProcessorInfo.RAWProcessorInformation^.L2CacheHandle > 0)
            and (LProcessorInfo.L2Chache <> nil)  then
            begin
          ...
          if (LProcessorInfo.RAWProcessorInformation^.L3CacheHandle > 0)
            and (LProcessorInfo.L3Chache <> nil) then
            begin
